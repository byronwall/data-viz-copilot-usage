...
def _choose_port(host: str, requested_port: int | None) -> int:
    if requested_port is not None:
        return requested_port
    for port in range(5057, 5078):
        if _is_port_available(host, port):
            return port
    with socket.create_server((host, 0), reuse_port=False) as sock:
...
def main():
    ...
    if not args.no_open:
        threading.Timer(0.75, _open_browser, args=(url,)).start()
