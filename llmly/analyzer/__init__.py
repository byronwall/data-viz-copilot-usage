...
# Per-source daily aggregate caches are persisted in a user-writable cache dir so
# installed wheels do not try to write inside site-packages.
_CACHE_ROOT = os.path.expanduser(
    os.environ.get("LLMLY_CACHE_DIR")
    or os.path.join(os.environ.get("XDG_CACHE_HOME", "~/.cache"), "llmly")
)
os.makedirs(_CACHE_ROOT, exist_ok=True)
DAILY_CACHE_PATH = os.path.join(_CACHE_ROOT, ".daily_aic_cache.json")
DAILY_CODEX_CACHE_PATH = os.path.join(_CACHE_ROOT, ".daily_codex_cache.json")
DAILY_CLAUDE_CACHE_PATH = os.path.join(_CACHE_ROOT, ".daily_claude_cache.json")
